...
import xyz.jmullin.drifter.FloatMath
...
class RichVector2(v: Vector2) {
  ...
  def flipX = V2(gameW) + (v * V2(-1, 1)).fixZeroes
  def flipY = V2(0, gameH) + (v * V2(1, -1)).fixZeroes
  def inverseX = (v * V2(-1, 1)).fixZeroes
  def inverseY = (v * V2(1, -1)).fixZeroes
  ...
  def min(o: V2) = V2(math.min(v.x, o.x), math.min(v.y, o.y))
  def max(x: Float, y: Float) = V2(math.max(v.x, x), math.max(v.y, y))
  def max(o: V2) = V2(math.max(v.x, o.x), math.max(v.y, o.y))
  def clamp(a: V2, b: V2) = V2(FloatMath.clamp(v.x, a.x, b.x), FloatMath.clamp(v.y, a.y, b.y))
